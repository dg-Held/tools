...
Aufruf (normalerweise über die BAT-Datei):
  python inca_year_precompute.py --year 2026 --input "C:\INCA\2026" \
      --climate-dir "..\data\climate-precomputed"
...
def discover_targets(climate_dir: Path) -> tuple[list[TargetPoint], dict[str, list[int]]]:
    ...
    if not targets:
        raise SystemExit(
            "Keine bestehenden Klimaprofile in climate-precomputed/tiles gefunden. "
...
def main() -> int:
    ...
    parser.add_argument("--climate-dir", type=Path, required=True, help="Ordner data/climate-precomputed")
