...
function applyAddressResult(address) {
  ...
  elements.addressSearchStatus.textContent =
    address.is_demo
      ? 'Technischer Demonstrationspunkt übernommen. Der vollständige BEV-Tirol-Index wird mit dem beiliegenden Importer erzeugt.'
      : 'Offizielle Adresse übernommen. Koordinaten und Adresscode stammen aus dem lokalen BEV-Stichtagsindex.';
...
function readSelectedLocation() {
  ...
  return {
    ...
    nat_assignment_status: coordinatesChanged
      ? 'Manuell übernommener Testwert; Katastralgemeinde noch nicht automatisch zugeordnet.'
...
async function loadLocation(location, progressState) {
  ...
  for (let year = START_YEAR; year <= END_YEAR; year += 1) {
    ...
    setProgress(
      progressState.done,
      progressState.total,
      `${location.name}: ${year} ${result.fromCache ? 'aus Zwischenspeicher' : 'geladen'}`
    );
  }
  ...
  const climateResult = ClimateCore.analyzeLocation(
    {
      ...location,
      grid_longitude: gridCoordinates?.[0] ?? null,
      grid_latitude: gridCoordinates?.[1] ?? null,
...
function downloadJson() {
  ...
  const payload = Object.keys(state.results).length > 1
    ? { schema_version: 11, results: state.results }
    : state.currentResult;
