...
(function initEnvelopeRenovationTool(global) {
  ...
  function automaticInputsForComponent(project, component) {
    ...
    return {
      ...
      heatingDegreeHoursKh: climate.heatingDegreeHoursKh
        ?? (finite(draft.heatingDegreeDaysKd, HGT_FALLBACK_TIROL) > 0
          ? finite(draft.heatingDegreeDaysKd, HGT_FALLBACK_TIROL) * 24
          : null),
      natC: climate.natC,
  ...
  function buildMeasureRecord(inputs, selected, options = {}) {
    ...
    return {
      ...
      generatedBy: options.autoGenerated ? 'Bauteil & Sanierung V1.0' : null,
      thermalEnvelope: {
  ...
  function renderFromProject(project) {
    ...
    if (flowComponent?.lossKwh > 0) {
      ...
      $('energyBasisTitle').textContent = 'HGT-Fallback aktiv';
      $('energyBasisText').textContent = `${formatNumber(hgtValue, 0)} Kd/a × 24 · überschlägige U × A-Rechnung; Standortklima kann die Grundlage verbessern.`;
      $('calculateClimate').hidden = false;
    } else {
      $('climateStrip').dataset.level = 'warning';
      $('energyBasisTitle').textContent = 'Energiegrundlage fehlt noch';
      $('energyBasisText').textContent = 'Energiefluss berechnen, Klimawerte hier laden oder HGT manuell ergänzen.';
      $('calculateClimate').hidden = false;
    }
  ...
  function currentInputs() {
    ...
    return {
      ...
      existingLossKwh: finite(flowComponent?.lossKwh, null),
      heatingDegreeHoursKh: climate.heatingDegreeHoursKh
        ?? (inputNumber('heatingDegreeDays', null) > 0
          ? inputNumber('heatingDegreeDays', null) * 24
          : null),
      natC: climate.natC,
      period: climate.period,
  ...
  function persistVisibleInputs() {
    ...
    if (constructionYearInput?.dataset.userEdited === 'true') {
      ...
      else store.setFieldCandidate('building.profile.constructionYear', model.ORIGIN.MANUAL, Math.round(inputs.constructionYear), { unit: 'Jahr', source: 'Nutzereingabe Bauteil & Sanierung V1.0' });
    ...
    if (usableFloorAreaInput?.dataset.userEdited === 'true') {
      if (inputs.usableFloorAreaM2 > 0) {
        store.setFieldCandidate('building.geometry.usableFloorArea', model.ORIGIN.MANUAL, inputs.usableFloorAreaM2, { unit: 'm²', source: 'Nutzereingabe Bauteil & Sanierung V1.0' });
      } else {
    ...
    if (component.areaPath && (areaInput?.dataset.userEdited === 'true' || doorAreaEdited)) {
      if (inputs.areaM2 > 0) store.setFieldCandidate(component.areaPath, model.ORIGIN.MANUAL, inputs.areaM2, { unit: 'm²', source: 'Bauteil & Sanierung V1.0', method: isDoorComponent(component) ? 'Anzahl × typische Fläche je Haustür' : null });
    ...
    if (component.uPath && uInput?.dataset.userEdited === 'true') {
      if (inputs.existingUValue > 0) store.setFieldCandidate(component.uPath, model.ORIGIN.MANUAL, inputs.existingUValue, { unit: 'W/m²K', source: 'Bauteil & Sanierung V1.0' });
      else store.clearFieldCandidate(component.uPath, model.ORIGIN.MANUAL);
      uInput.dataset.userEdited = 'false';
    }

    store.setFieldCandidate('systems.heating.usefulHeatFactor', model.ORIGIN.MANUAL, inputs.annualEfficiency, { source: 'Bauteil & Sanierung V1.0' });
    store.setFieldCandidate('building.thermal.indoorTemperature', model.ORIGIN.MANUAL, inputs.indoorTemperatureC, { unit: '°C', source: 'Bauteil & Sanierung V1.0' });
    writeDraft({
      lambdaWmk: inputs.lambdaWmk,
      frameMaterial: inputs.frameMaterial,
