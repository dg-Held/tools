...
(function initStandortpassV1() {
  ...
  function heatTone(label, value, note = '') {
    ...
    /* Ein expliziter Nicht-Treffer ist neutral – auch wenn die Kartenbezeichnung
       Wörter wie Schutzgebiet oder Beschränkung enthält. */
    if (/0 treffer|kein treffer|kein direkter flächentreffer|keine/.test(combined)) return 'neutral';
    if (/offen|nicht verfügbar|nicht geprüft|fehlgeschlagen|fehler/.test(combined)) return 'open';
    if (/wärmenetz|bewilligungspflicht|beschränkung|schutz.*treffer/.test(combined)) return 'attention';
