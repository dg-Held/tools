...
function applyNatForAddress(address) {
  const result = OibNatCore.lookupAddress(address);
  ...
  if (result.status === 'exact') {
    ...
    state.selectedNatReference = reference;
    elements.natInput.value =
      reference.nat_at_elevation_min_c;
    elements.natReferenceHeightInput.value =
      reference.elevation_min_m;
    ...
    return {
      ...
      text:
        ...
        `${formatNumber(
          reference.nat_at_elevation_min_c,
          1
        )} °C · KG ${reference.kg_number} ` +
        `${reference.kg_name} · ELEVMIN ` +
        `${formatNumber(reference.elevation_min_m)} m.`,
    };
  }
...
function readSelectedLocation() {
  ...
  const natC = Number(elements.natInput.value);
  const natReferenceHeight =
    Number(elements.natReferenceHeightInput.value);
...
async function loadLocation(location, progressState) {
  ...
  /*
    Schnellweg:
    14 vollständige Jahre ergeben bei einem Parameter rund 123.000 Werte.
    Das liegt deutlich unter dem GeoSphere-JSON-Limit von 1.000.000 Werten.
    Deshalb wird der ganze Zeitraum zuerst mit einer Anfrage geladen.
  */
  try {
    setProgress(
      progressState.done,
      progressState.total,
      `${location.name}: Klimadaten 2012–2025 werden gemeinsam geladen …`
    );
    ...
    const sourceText = {
      'period-cache': 'aus Perioden-Zwischenspeicher',
      'legacy-cache': 'aus vorhandenem Jahres-Zwischenspeicher',
      'period-api': 'mit einer GeoSphere-Abfrage geladen',
...
function renderLocationCheck(result) {
  ...
  elements.locationFacts.innerHTML = [
    ...
    locationFact(
      ...
      'Aufbereitung der aktuellen OIB-Datei folgt'
    ),
  ].join('');
...
function renderChart(result) {
  ...
  const xMax = Math.ceil((Math.max(dataMaximum + 2, 32)) / 5) * 5;
...
function downloadJson() {
  ...
  const payload = Object.keys(state.results).length > 1
    ? { schema_version: 13, results: state.results }
    : state.currentResult;
