...
function applyOibReferencesForAddress(address) {
  const natResult = OibNatCore.lookupAddress(address);
  const tnatResult = OibTnat13Core.lookupAddress(address);

  state.selectedNatReference = null;
  state.selectedTnat13Reference = null;

  if (
    natResult.status === 'exact' &&
    tnatResult.status === 'exact'
  ) {
    const natReference = natResult.reference;
    const tnatReference = tnatResult.reference;
    ...
    return {
      ...
      text:
        ...
        `${formatNumber(
          tnatReference.tnat13_at_elevation_min_c,
          1
        )} °C · KG ${natReference.kg_number} ` +
        `${natReference.kg_name} · ELEVmin ` +
        `${formatNumber(natReference.elevation_min_m)} m.`,
    };
  }
...
function downloadJson() {
  ...
  const payload = Object.keys(state.results).length > 1
    ? { schema_version: 14, results: state.results }
    : state.currentResult;
