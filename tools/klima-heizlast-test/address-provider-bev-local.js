...
(function initBevLocalAddressProvider(global) {
  ...
  class BevLocalAddressProvider {
    ...
    async loadChunk(postalCode) {
      if (this.chunkCache.has(postalCode)) {
        return this.chunkCache.get(postalCode);
      }
      ...
      this.chunkCache.set(postalCode, standardized);
      return standardized;
    }
    ...
    async search(query, { limit = 8 } = {}) {
      ...
      const chunks = await Promise.all(
        postalCodes.map((postalCode) => this.loadChunk(postalCode))
      );
